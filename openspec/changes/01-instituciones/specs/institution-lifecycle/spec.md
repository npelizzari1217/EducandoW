...
Change `01-instituciones` extends institution lifecycle to cover the complete 25-field model, adds the `contact_email` rename, adds `address` fields (city, postal_code, country, ministry_reg, cue, website), and updates `GET /v1/institutions` to support active-status filtering. All existing requirements are preserved; the delta adds the new fields, rename, and filter behavior formally.
...
(Previously: accepted only 5 fields — name, email, phone, address, levels)
...
(Previously: no active query filter — always returned all institutions)
...
(Previously: response included only 5 fields)
...
(Previously: `cue` not listed as ADMIN-restricted; only 5 fields existed)
