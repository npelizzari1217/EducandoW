...
Change `01-instituciones` replaces the singleton `PrismaService` with a dynamic factory pattern and introduces `TenantMiddleware` as the routing gate. The existing spec already defines JWT `dbName`, the factory Map, and master DB isolation. This delta adds the schema-split constraint, the TenantMiddleware application paths, and the active-status gate within middleware.
...
(Previously: active check at login was not in the multi-tenant-routing spec — it was only in institution-lifecycle)
...
(Previously: middleware attachment and lazy instantiation were implicit — now explicit)
...
(Previously: unchanged in routing behavior; active check is now added as a co-gate — see ADDED requirement above)
