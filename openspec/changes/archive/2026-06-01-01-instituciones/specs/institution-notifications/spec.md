# Delta for institution-notifications

## Context

Change `01-instituciones` integrates notification toggle fields into the full 25-field Institution model. The existing spec already covers `send_messages`, `socket_host`, and `socket_port`. This delta formally incorporates `send_email` into the notifications capability (previously only covered by institution-smtp) and aligns feature-flag UI behavior to the complete field set.
...
(Previously: `send_email` was not part of the notifications capability — it only appeared in institution-smtp)
...
(Previously: `send_email` was not in this list — it was only in institution-smtp)
