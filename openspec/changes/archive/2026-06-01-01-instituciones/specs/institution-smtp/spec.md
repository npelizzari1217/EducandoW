# Delta for institution-smtp

## Context

Change `01-instituciones` tightens the SMTP spec by: (1) making the `ENCRYPTION_KEY` startup check explicit as an application bootstrap gate, (2) clarifying that `smtp_pass` MUST be modelled as a `SmtpConfig` Value Object in the domain layer rather than a raw string, and (3) confirming that `send_email` ownership moves to `institution-notifications` while the SMTP spec retains the encryption mechanics.
...
(Previously: fields accepted as raw strings — no domain VO enforcing constraints at entity construction)
...
(Previously: startup check was implied; not explicitly scoped to bootstrap)
...
(Previously: `send_email` was fully owned by this spec — it now delegates flag storage to institution-notifications)
