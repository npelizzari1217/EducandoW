# Delta for institution-branding

## Context

Change `01-instituciones` extends the branding spec to cover the full 25-field Institution model. The three color fields and `logo_url` already exist. This delta adds the hex validation Value Object (`HexColor`) and integrates branding fields into the complete field set, while confirming that the `PATCH` endpoint is the sole mutation path.
...
(Previously: fields validated only at DTO layer with regex; no domain VO)
