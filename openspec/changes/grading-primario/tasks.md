...
| PR4-T8 | [x] [GREEN] Implement `UpsertSubjectPeriodGrades` use case (one write path for both period grades AND pa/ppi/pp flags — no separate PATCH endpoint per AD-3; batch upsert via `saveMany`) | `api/src/application/grading/upsert-subject-period-grades.use-case.ts` | SPG-R3..R9, PPF-R4, AD-3 |
...
| PR4-T10 | [x] [GREEN] Implement `UpsertSubjectFinalGrades` use case (conditional lifecycle enforced HERE, not in entity; `FINAL` eager-scaffolded; `DICIEMBRE/MARZO/DEFINITIVA` on-demand) | `api/src/application/grading/upsert-subject-final-grades.use-case.ts` | SFG-R3..R9, AD-2 |
...
| PR4-T15 | [x] Define Zod query-param schemas (read-side) + write-side DTOs: `UpsertSubjectPeriodGradesSchema`, `UpsertSubjectFinalGradesSchema` (PR4b) | `api/src/presentation/grading/dto/subject-grades.dto.ts` | SPG-R9, SFG-R8 |
| PR4-T16 | [x] Write failing controller tests for `SubjectGradesController`: read-side GET tests (PR4a) + write-side PUT tests (PR4b): PUT period grades → 200 `{ data: null }`; PUT finals → 200 `{ data: null }`; ValidationError → 400; NotFoundError → 404 | `api/src/presentation/grading/subject-grades.controller.spec.ts` | SPG-R8, SFG-R10, TIA-R8, SPG-R3, SFG-R5 |
| PR4-T17 | [x] Implement `SubjectGradesController` full: GET endpoints (PR4a) + PUT period grades + PUT finals (PR4b) | `api/src/presentation/grading/subject-grades.controller.ts` | SPG-R8, SFG-R10, TIA-R8, ES-R7, SPG-R3, SFG-R5 |
...
| ID | Action | Files | Spec refs |
|---|---|---|---|
| PR5-T1 | [x] [RED] Write failing component tests for `TeacherFilteredSelector`: calls `GET /course-cycles?teacherUserId=` and `…/subjects?teacherUserId=`; renders CC dropdown and subject dropdown; shows empty state when no CC returned | `web/src/pages/dashboard/__tests__/TeacherFilteredSelector.test.tsx` | TIA-R3, TIA-R4, ES-R4, ES-R6 |
| PR5-T2 | [x] [GREEN] Implement `TeacherFilteredSelector` component (teacher-filtered 2-level selector: CC → subject; empty-state on empty response; reuses existing dropdown primitives) | `web/src/pages/dashboard/components/TeacherFilteredSelector.tsx` | TIA-R3, TIA-R4, ES-R4, ES-R6 |
| PR5-T3 | [x] [RED] Write failing hook tests for extended `use-grading-grid`: adds subject-period-grade + final-grade + flags fetch/save channels alongside existing competency channel; dense Map includes period grades + finals + flags; bounded-parallel save unchanged; returns ALL competencies with `imprimible` field per row (no pre-filter) | `web/src/pages/dashboard/__tests__/use-grading-grid.test.ts` (describe block `useGradingGrid - subject-grade channels`, SGC-1..SGC-10) | ES-R7, ES-R8, AD-3 (CORRECTION applied) |
| PR5-T4 | [x] [GREEN] Extend `use-grading-grid` hook: add subject-grade fetch/save, finals fetch/save, flags fetch/save; expose `imprimible` boolean per competency row without any filter | `web/src/pages/dashboard/components/use-grading-grid.ts` | ES-R7, ES-R8 |
| PR5-T5 | [x] [RED] Write failing integration tests for `SubjectGradingBySubject` page: shows ALL competencies with "Imprimir" toggle per row (not pre-filtered); shows period-grade dropdowns per period tab; shows PA/PPI/PP toggle per student per period; inline save with no full-page reload; empty state when teacher has no assignments; Primario-only (non-Primario CC not shown). NOTE: SBS tests mock CompetencyGradingGrid; CGG-13 tests (competency-grading-grid.test.tsx) provide non-mocked toggle coverage. | `web/src/pages/dashboard/__tests__/subject-grading-by-subject.test.tsx`, `web/src/pages/dashboard/__tests__/competency-grading-grid.test.tsx` | ES-R1 (CORRECTED), ES-R4, ES-R6, ES-R7, ES-R8, ES-R10, ES-R11 |
| PR5-T6 | [x] [GREEN] Implement `SubjectGradingBySubject` page: ALL competencies displayed + "Imprimir" toggle per row; period-grade dropdowns; 4 final-grade cells; PA/PPI/PP toggle per student×period; `TeacherFilteredSelector`; Primario-only. ALSO: `imprimible` write path added — `UpdatePeriodGradeSchema` extended with `imprimible?: boolean`; `GradePeriodValuationUC` calls `setImprimible` when present; `CompetencyGradingGrid` renders checkbox per cell with PATCH on change; `updateImprimible` added to `useGradingGrid` hook. CGG-13 tests (4 tests, RED→GREEN) confirm toggle exercises real component. | `web/src/pages/dashboard/subject-grading-by-subject.tsx`, `web/src/pages/dashboard/components/CompetencyGradingGrid.tsx`, `web/src/pages/dashboard/components/use-grading-grid.ts`, `api/src/presentation/pedagogy/dto/competency.dto.ts`, `api/src/application/pedagogy/use-cases/competency.use-cases.ts`, `api/src/presentation/pedagogy/pedagogy.controller.ts` | ES-R1 (CORRECTED), ES-R4, ES-R7, ES-R8, ES-R10, ES-R11 |
| PR5-T7 | [x] Update router: replace `/competency-grading` route with `SubjectGradingBySubject` | `web/src/App.tsx` | ES-R3 |
...
| ID | Action | Files | Spec refs |
|---|---|---|---|
| PR7-T1 | [x] [RED] Write failing regression test: for any CC where `Math.floor(level/10) !== 2`, `generateBoletin` reads the `NotaTrimestral` path and output is byte-for-byte equivalent to pre-change behavior | `api/src/application/reportes/__tests__/generate-boletin.use-case.test.ts` | BP-R2, AD-8 |
| PR7-T2 | [x] [RED] Write failing tests for `buildMateriasPrimario`: reads `SubjectGradingPeriod` (dynamic period columns from snapshotted `periodName`); reads `SubjectPeriodGrade` (period grades + pa/ppi/pp); reads `SubjectFinalGrade` (4 instances; absent row → blank not error); reads `CompetencyPeriodValuation` filtered to `imprimible=true`; OR-aggregates pa/ppi/pp flags across reported periods per subject | `api/src/application/reportes/__tests__/generate-boletin.use-case.test.ts` | BP-R1, BP-R3, BP-R4, BP-R5, BP-R6, BP-R8, BP-R9, AD-8 |
| PR7-T3 | [x] [GREEN] Extend `MateriaBoletin` type with optional fields `periodGrades?`, `finalGrades?`, `competencies?`, `flags?` — legacy path leaves them `undefined`, non-Primario templates must not crash | `api/src/application/reportes/templates/boletin.template.ts` | BP-R7, AD-8 |
| PR7-T4 | [x] [GREEN] Implement `buildMateriasPrimario(courseCycleId, institutionId)` reading the four data sources + level dispatch (`Math.floor(level/10)===2` → Primario branch; else → existing `NotaTrimestral` code path untouched) in `generate-boletin.use-case.ts` | `api/src/application/reportes/generate-boletin.use-case.ts` | BP-R1, BP-R2, BP-R5, BP-R6, BP-R8, AD-8 |
| PR7-T5 | [x] Rebuild `boletin-primario.hbs`: dynamic period columns from `periodGrades[].periodName`; finals section (4 instances, blank if absent); competency section renders competencies already filtered `imprimible=true` by use case; flags section per subject row (false flags hidden from output) | `api/src/infrastructure/reporting/html-templates/boletin-primario.hbs` | BP-R3, BP-R4, BP-R5, BP-R6, BP-R9, AD-8 |
| PR7-T6 | [x] Mark `CalificacionPrimario` and `NotaTrimestral` as `@deprecated` via JSDoc (NOT deleted — Secundario/Terciario boletines still use them) | `packages/domain/src/primario/entities/calificacion-primario.ts`, `packages/domain/src/pedagogy/entities/nota-trimestral.ts` | AD-8 |
