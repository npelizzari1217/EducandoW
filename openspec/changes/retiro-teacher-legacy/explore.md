...
- **B3 — Migrar modo homeroom + drop `Teacher` table + `SubjectAssignment.teacherId` FK (~350, MEDIO):** refactor homeroom a AsignacionCursoXCiclo rol=TITULAR; quitar `findByHomeroomTeacher`; decidir destino de la página /teachers; dropear tabla `Teacher` y FK `teacherId`. Requiere verificar backfill Fase 4 completo por tenant. El drop de `Teacher` está ahora DESBLOQUEADO por S2 (ya no hay lecturas del boletín).
  **STATUS: PENDING — depende de S2 (completo). Decisions #2/#3 (Teacher track) deben resolverse primero. NO incluye drop de `SubjectAssignment` — ver S3-pre.**
