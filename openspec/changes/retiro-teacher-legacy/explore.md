...
1. **Evaluacion / NotaTrimestral** (historial de notas legacy): **RESUELTO 2026-06-17 → ARCHIVAR + DROPEAR.** Confirmado que NotaTrimestral es read-only histórico (CRUD removido en S1; solo lo lee el boletín Inicial/Terciario). S3-pre: exportar Evaluacion/NotaTrimestral a archivo (dump/tabla cold) → repuntar el boletín Inicial/Terciario al modelo nuevo (SubjectPeriodGrade/SubjectFinalGrade; estrategias ya existen para todos los niveles) → dropear las tablas legacy + SubjectAssignment → habilita S3b-final (drop Teacher).
...
S1, S2, S3a, **S3b-0**, **S3b-1**, **S3b-2**, **S3b-3** DONE. Decisions #2/#3/#4 resueltas (retiro total de Teacher). **R-GAP S3b-2 CERRADO** (2026-06-17): exam boards/actas necesitan solo un User.id. Consumidor Teacher restante: SOLO `SubjectAssignment.teacherId` (FK Cascade), gated por **S3-pre** (Decision #1 RESUELTA 2026-06-17 → ARCHIVAR+DROPEAR). Orden siguiente: **S3-pre** (archivar Evaluacion/NotaTrimestral + repuntar boletín Inicial/Terciario al modelo nuevo + dropear tablas legacy + SubjectAssignment) → **S3b-final** (drop tabla Teacher + entidad de dominio + repo + index exports). `CourseCycle.homeroomTeacherId` ya no existe (dropped S3b-0). `Sala/Grado/Curso.teacherId` ya no existe (dropped S3b-1). `/teachers` admin ya no existe (retired S3b-2). `MesaExamen/ActaExamen.presidenteId` ya no tiene FK a Teacher (migrado S3b-3).
