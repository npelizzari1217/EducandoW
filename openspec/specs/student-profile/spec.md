...
### Requirement: fatherEmail / motherEmail on Student (REQ-RYT-01)

> Added: responsables-y-tutores (2026-06-29). Archive: `openspec/changes/archive/2026-06-29-responsables-y-tutores/`.

`Student` MUST expose two optional email fields: `fatherEmail` and `motherEmail`. Both fields MUST be validated as `Email` VO when a value is present and MUST accept `undefined`/`null` when absent. These fields MUST NOT appear in `ALLOWED_TUTOR_FIELDS` — they are writable only by ADMIN, ROOT, or equivalent privileged roles. These emails belong to the student's institutional file (legajo) and are independent of any tutor contact email stored in `StudentGuardian.email`.
