...
>   asistencia-autollenado-p (archived 2026-07-02) — ATR-R11
> IDs: ATR-R* / ATR-S*
> Cross-references:
...
### ATR-R11 — Autollenado de Presente en días hábiles vacíos al Generar (asistencia-autollenado-p, 2026-07-02)

> Added: asistencia-autollenado-p (2026-07-02). Archive: `openspec/changes/archive/2026-07-02-asistencia-autollenado-p/`.
> **Nota de reconciliación (sdd-archive):** la spec delta original de este change definía la resolución
> del Presente (ATR-R11.5) en 4 ramas, incluyendo una rama de ambigüedad (`PresenteTypeAmbiguousError` /
> `PRESENTE_TYPE_AMBIGUOUS`, HTTP 422) para 2+ `AttendanceType` custom con `behavior = NO_COMPUTA` sin
> `code = "P"`, y el escenario ATR-S79 que la ejercitaba. La implementación real (`findPresenteByLevel`,
> ver ADR-2 en `design.md` del change archivado) resuelve el Presente con UNA sola consulta
> determinística — `isPresent = true AND isSystem = true AND active = true AND deletedAt = null` para
> el nivel — unívoca por construcción (`@@unique([level, code])` + protección
> `ATTENDANCE_TYPE_SYSTEM_PROTECTED` sobre tipos de sistema, ver `attendance-types/spec.md`). Esa
> ambigüedad de negocio nunca puede ocurrir con esta implementación: no existe un segundo
> `AttendanceType` de sistema con `isPresent=true` para el mismo nivel. El fallback a un
> `AttendanceType` custom con `behavior = NO_COMPUTA` (ex-ramas 1/2 del delta original) tampoco se
> implementó — la consulta real filtra `isSystem = true`, por lo que un custom jamás puede resolverse
> como Presente. Por eso ATR-R11.5 se documenta abajo con DOS ramas (encontrado / no encontrado), y
> `PRESENTE_TYPE_AMBIGUOUS` NO existe en el repo. Detalle completo:
> `sdd/asistencia-autollenado-p/archive-report` (engram, project `educandow`).
...
(`AttendanceMonthStatus`, ATR-R10) NO cambia por este cambio. El autollenado hereda el
comportamiento actual de Generar sobre mes cerrado, sea cual sea ese comportamiento — este cambio
MUST NOT introducir un nuevo código de error ni un nuevo bypass para ese caso. Bajo NINGUNA
circunstancia el paso de autollenado (`fillHabilVacios` o equivalente) MUST ejecutar una escritura
efectiva sobre un (C, Y, M) CLOSED. Implementación verificada: guard reutiliza el `existingStatus`
ya leído (paso previo del use-case); si `isClosed()`, se saltea la resolución de Presente y no se
escriben `days` — sin error nuevo, sin bypass.
...
- AND el nivel SECUNDARIO tiene un `AttendanceType` de sistema con `isPresent = true`
...
jamás puede resolverse como Presente. Ver nota de reconciliación al inicio de ATR-R11.
...
`PRESENTE_TYPE_AMBIGUOUS` en el repo. Ver nota de reconciliación al inicio de ATR-R11.
...
Registrado en `DOMAIN_STATUS` del `AppExceptionFilter`, consistente con `DayNotAssignableError`/`StatusNotAssignableError` (ATR-R8).

**Deuda de verificación conocida (pendiente al momento del archive):** los tests de integración
`api/test/integration/asistencia/generate-monthly.db.test.ts` (GEN-DB-01..10, cubren ATR-S71..S82
incluido el invariante de no-sobrescritura end-to-end contra Postgres real) están ESCRITOS pero NO
CORRIDOS — no hay Postgres tenant (`:5433`) disponible en ningún entorno donde se ejecutó verify.
Deben correrse contra DB real antes de considerar este capability 100% probado end-to-end.

---

## ADR cross-reference (asistencia-autollenado-p, 2026-07-02)

| ADR | Decision | Satisfies |
|-----|----------|-----------|
| ADR-1 | Merge fill-only (`{ ...incoming, ...existing }`, existing gana) reemplaza el merge locked-wins previo — única forma de topear hábiles vacíos en filas existentes sin pisar al docente. **Sign-off del usuario**: aprobado verbalmente durante la fase de planificación (registrado en archive-report, no en un artefacto SDD previo). | ATR-R11.2, ATR-R11.3 |
| ADR-2 | Resolver Presente por `isPresent && isSystem` (no por string `"P"` hardcodeado, no por `behavior` solo) — unívoco por construcción | ATR-R11.5 |
| ADR-3 | `Result<T,E>` para `PresenteTypeNotFoundError`; throws legacy del use-case (`ForbiddenError`, `NotFoundError`, `PreviousMonthOpenError`) intactos — método mixto transicional | ATR-R11.5 |
| ADR-4 | Guard de mes CERRADO: saltear autollenado (no-op silencioso), reusa `existingStatus` ya leído — no se agrega error ni bypass nuevo | ATR-R11.6 |
