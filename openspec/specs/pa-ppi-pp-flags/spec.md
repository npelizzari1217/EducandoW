...
> **Implementation note (AD-3)**: The behaviors in this spec are delivered via `SubjectPeriodGrade`,
> NOT via a separate `PedagogicalFlag` entity or endpoint. Per design decision AD-3, the three
> flags (`pa`, `ppi`, `pp`) are `Boolean @default(false)` columns on `SubjectPeriodGrade`.
> They are saved and retrieved through `PUT /grading/subject-grades` (not a dedicated PATCH
> /pedagogical-flags endpoint). The `GET /pedagogical-flags` endpoint described in PPF-R10
> is NOT implemented; flag data is returned as fields on each `SubjectPeriodGrade` row via
> `GET /grading/subject-grades?courseCycleId=&subjectId=`. The behavioral requirements
> (PPF-R1 through PPF-R11) remain valid requirements — only the delivery mechanism differs.
